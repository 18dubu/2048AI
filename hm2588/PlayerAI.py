...
class PlayerAI(BaseAI):
    ...
    def search1(self, grid, depth, alpha, beta, positions, isPlayer =True, errorLog=False):
        self.grid = grid
        args = [grid, depth, alpha, beta, positions, isPlayer,'False']
        # the maxing player
        bestMove = -1
        ...
        if isPlayer:
            ...
            for move in grid.getAvailableMoves():
                ...
                shouldAddPenalty = 0
                if 1:
                    if self.isBigTileInCorner(grid) and self.willMoveLetBiggestTileOffCorner(move,newGrid):
                        shouldAddPenalty = 1
                    newGrid.move(move)
                    newGrid.insertTile(self.eval_fn2(newGrid)[1],2)
                    positions += 1
                    if depth == 0:
                        result = {'move': move, 'score': self.eval_fn2(newGrid)[0], 'positions': positions}
                    else:
                        result = self.search1(newGrid, depth-1, bestScore, beta, positions, True)
                        positions = result['positions']
                        if shouldAddPenalty:
        ...
        # computer turn
            ...
            if 0:
                ...
                for i in range(len(candidates)):
                    ...
                    newGrid.insertTile(position, value)
                    positions += 1
                    result = self.search1(newGrid, depth, alpha, bestScore, positions, True)
                    positions = result['positions']
                    if result['score'] < bestScore:
                        bestScore = result['score']
                        bestMove = result['move']
    ...
    def getMove(self, grid):
        ...
        if method == 'categoricalDeep':
            ...
            if emptyCellNum <= 1:
                depth = 4
            self.move = int(self.search1(grid,depth,-infinity,infinity,0,True)['move'])
